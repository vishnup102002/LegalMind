...
5. NO REPETITIVE LOOPS: Every sentence must contain unique legal facts."""
        ...
        try:
            ...
            # Post-generation string sanitizer to eliminate any leaked bracket placeholders
            draft = re.sub(r'\[\s*TO BE INSERTED\s*\]', 'As per employment records', draft, flags=re.IGNORECASE)
            draft = re.sub(r'\[\s*TO BE FILLED BY SENDER\s*\]', 'As per employment records', draft, flags=re.IGNORECASE)
            draft = re.sub(r'\[\s*അയക്കുന്നയാൾ പൂരിപ്പിക്കേണ്ടതാണ്\s*\]', 'രേഖകൾ പ്രകാരം', draft, flags=re.IGNORECASE)
            draft = re.sub(r'\[\s*[^\]]+rupees\s*\]', 'three (3) months salary arrears', draft, flags=re.IGNORECASE)
        except Exception as e:
...
{draft}
    ...
    def parse_legal_notice_slots(self, text: str) -> Dict[str, Optional[str]]:
        """Parses key-value form fields for legal notice drafting with Pydantic role validation gate."""
        sender_name = None
        recipient_name = None
        company_name = None
        ...
        for line in lines:
            ...
            # Check Sender / Employee Name Field
            # Strictly match 'നിങ്ങളുടെ പൂർണ്ണ പേര്', 'പൂർണ്ണ പേര്', 'അയക്കുന്നയാൾ', 'sender', 'from', 'complainant'
            if re.search(r'^(?:നിങ്ങളുടെ\s*)?പൂർണ്ണ\s*പേര്\s*[:：]|^(?:അയക്കുന്നയാൾ|ഫ്രം|sender|from|complainant)\s*[:：]', line_str, re.IGNORECASE):
                val = re.sub(r'^[^\:\：]+[:：]\s*', '', line_str).strip()
                if val and len(val) >= 2:
                    sender_name = val

            # Check Recipient / Employer / Company Field
            elif re.search(r'^(?:[തൊൊ]ഴിലുടമയുടെ\s*പേര്|കമ്പനിയുടെ\s*പേര്|കമ്പനി|സ്വീകരിക്കുന്നയാൾ|ലഭിക്കേണ്ടയാൾ|എതിർകക്ഷി|recipient|respondent|to|receiver)\s*[:：]', line_str, re.IGNORECASE):
                val = re.sub(r'^[^\:\：]+[:：]\s*', '', line_str).strip()
                if val and len(val) >= 2:
                    if "കമ്പനി" in line_str or "company" in line_str.lower():
                        company_name = val
                    else:
                        recipient_name = val

        # Combine employer name and company name for recipient if both exist
        if recipient_name and company_name and company_name.lower() not in recipient_name.lower():
            final_recipient = f"{recipient_name} ({company_name})"
        elif recipient_name:
            final_recipient = recipient_name
        elif company_name:
            final_recipient = company_name
        else:
...
12. NO FAKE OR HALLUCINATED URLS (STRICT RULE):
    - NEVER invent, output, or hallucinate fake links like 'example.com', 'https://example.com/legalnotice', or placeholder HTTP links!
    - The ONLY valid download tag is the exact '[DOWNLOAD_URL:...]' tag generated by the 'draft_legal_notice' tool when required details are provided.
    - If required details are missing, DO NOT output any URL — simply ask the user to provide their full name and company name in {lang_name}.
13. STRICT STATUTORY CITE MANDATE:
    - NEVER cite fake or non-existent laws like 'നിയമസഭ നിയമം 25' (Legislative Act 25) or 'ഇന്ത്യൻ കോടതി നിയമം 1973'!
    - For non-payment of wages, ONLY cite: 'പേയ്‌മെന്റ് ഓഫ് വേജസ് ആക്ട്, 1936 (സെക്ഷൻ 5)' and 'ഇൻഡസ്ട്രിയൽ ഡിസ്പ്യൂട്ട്സ് ആക്ട്, 1947 (സെക്ഷൻ 25)'.
...
- 'tenant' -> 'വാടകക്കാരൻ'"""
        ...
        # --- Deterministic Name Extraction with Pydantic Role Validation Gate ---
        slots = self.parse_legal_notice_slots(normalized_query)
        s_name = slots.get("sender_name")
        r_name = slots.get("recipient_name")
        
        if s_name and r_name:
            logger.info(f"Deterministic name extraction validated: sender='{s_name}', recipient='{r_name}' — bypassing LLM for direct tool call.")
        ...
        if not is_grounded:
            ...
            refined_query = f"{response_text} Payment of Wages Act 1936 Section 5"
            ...
            if second_context and "Statute [" in second_context:
                re_prompt = f"Synthesize a strict IRAC assessment for user in {retrieved_context} based ONLY on these verified statutes:\n{second_context}"
                try:
